...
#include <stdlib.h>
#include <stdio.h>  // 用于 FILE、popen、pclose 等函数
#include <sys/wait.h>
...
void lv_demo_widgets(void)
{
    ...
    lv_obj_t * t3 = lv_tabview_add_tab(tv, "music list");
    profile_create(t1);
    analytics_create(t2);
    shop_create(t3);
...
static void input_event_cb(lv_event_t * e) {
    ...
    if (code == LV_EVENT_FOCUSED) {
        ...
        strncpy(user_message_buffer, current_text, sizeof(user_message_buffer) - 1);
        user_message_buffer[sizeof(user_message_buffer) - 1] = '\0';  // 确保以空字符结尾
        printf("Debug: Updated input text: '%s'\n", user_message_buffer);
    }
}

// 发送按钮的事件回调函数
static void send_message_event_cb(lv_event_t * e) {
    printf("Debug: User message before send: '%s'\n", user_message_buffer);

    // 检查全局变量是否为空
    if (strlen(user_message_buffer) == 0) {
        lv_textarea_add_text(chat_log, "Error: Message is empty.\n");
    ...
    if (sock < 0) {
        lv_textarea_add_text(chat_log, "Error: Could not create socket.\n");
    ...
    server_address.sin_addr.s_addr = inet_addr("127.0.0.1");  // 或者使用服务器的 IP 地址

    if (connect(sock, (struct sockaddr *)&server_address, sizeof(server_address)) < 0) {
        lv_textarea_add_text(chat_log, "Error: Connection failed.\n");
    ...
    if (bytes_received > 0) {
        ...
        lv_textarea_add_text(chat_log, "AI: ");
        lv_textarea_add_text(chat_log, buffer);
        lv_textarea_add_text(chat_log, "\n");
        printf("AI: %s\n", buffer);
    } else {
        lv_textarea_add_text(chat_log, "Error: Failed to receive message.\n");
...
//    lv_obj_set_style_bg_img_src(parent, &background, 0);
 ...
 //   lv_obj_add_event_cb(lv_scr_act(), touch_event_cb, LV_EVENT_PRESSING, NULL); // 绑定全局触摸事件
    ...
    lv_obj_set_scroll_dir(cont, LV_DIR_VER);   // 设置滚动方向
...
for(i = 0; i < 10; i++) {
    ...
    lv_textarea_set_one_line(chat_input, true);  // 单行模式
    ...
    lv_obj_set_width(send_btn, lv_pct(20));  // 设置发送按钮的宽度为20%
    ...
    // 为发送按钮添加事件回调
    lv_obj_add_event_cb(send_btn, send_message_event_cb, LV_EVENT_CLICKED, NULL);
